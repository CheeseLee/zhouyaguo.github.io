...
- 如果availability\_zone中：后面的host名字不为空，则将host放入scheduler_hints，提醒scheduler要在指定的机器上生成。
- 准备好base_options
	...
	- nova.scheduler.multi.MultiScheduler.schedule\_run_instance
		- nova.scheduler.filter\_scheduler.FilterScheduler.FilterScheduler.schedule\_run_instance
			...
			- 筛选出符合用户请求的机器
				...
				- host\_manager.filter_hosts开始用很多filter来筛选
				- 筛选好后，将host consume resources，减去。为了让后面的申请依据最新的场景。
			- 每要新建一台，就调用weighted\_hosts.pop(0)一下，拿出一个host，调用provision\_resource开始做。
				...
				- 发送消息到queue中
				- 由于nova-compute启动时，创建了consumer，所以消息的处理位于ComputeManager.run_instance（见下节）


**nova-compute处理message过程**


即ComputeManager.run_instance：
...
- \_create_image
	- 生成libvirt.xml
	- \_cache_image
		- 调用libvirt\_utils.fetch\_image把远程image取到instance目录的_base目录下
		- copy\_and_extend
			- 从_base目录cp到独立的vm目录去
			- 如果申请的size大于镜像模板大小，则调用`qemu-img resize`来修改大小
		...
		- inject的方法就是将image mount到主机上，然后修改、拷贝。
